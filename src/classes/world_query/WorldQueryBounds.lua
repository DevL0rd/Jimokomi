local WorldQueryBounds = {}

WorldQueryBounds.getMapBounds = function(self, margin)
	margin = margin or 0
	return {
		left = margin,
		top = margin,
...
WorldQueryBounds.clampBoundsToMap = function(self, bounds, margin)
	margin = margin or 0
	if not bounds then
		return nil
	end
...
WorldQueryBounds.getViewBounds = function(self, margin)
	margin = margin or 0
...
WorldQueryBounds.getQueryBounds = function(self, rule)
	rule = rule or {}
	if rule.bounds then
...
WorldQueryBounds.getOffset = function(self, rule)
	rule = rule or {}
