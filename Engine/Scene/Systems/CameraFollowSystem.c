...
void CameraFollowSystem_Update(struct Scene* scene, float dt_seconds)
{
    ...
    entity = scene->camera_target_entity;
    ...
    if (camera_target == NULL || transform == NULL || !camera_target->follow)
    {
        ...
        scene->last_camera_follow_entity = NULL;
        return;
    }
    ...
    if (scene->last_camera_follow_entity == entity &&
        scene->last_camera_follow_target_x == target_x &&
        scene->last_camera_follow_target_y == target_y &&
        scene->last_camera_follow_view_width == scene->view.view_width &&
        scene->last_camera_follow_view_height == scene->view.view_height)
    {
    ...
    scene->last_camera_follow_entity = entity;
    scene->last_camera_follow_target_x = target_x;
    scene->last_camera_follow_target_y = target_y;
    scene->last_camera_follow_view_width = scene->view.view_width;
    scene->last_camera_follow_view_height = scene->view.view_height;
