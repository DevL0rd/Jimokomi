...
static bool spatial_grid_bounds_overlap(Aabb a, Aabb b)
{
...
{
    float nearest_x = clamp_f(center.x, bounds.min_x, bounds.max_x);
    float nearest_y = clamp_f(center.y, bounds.min_y, bounds.max_y);
...
static int spatial_grid_cell_x(const SpatialGrid* grid, float x)
{
    float local_x = 0.0f;
...
static int spatial_grid_cell_y(const SpatialGrid* grid, float y)
{
    float local_y = 0.0f;
