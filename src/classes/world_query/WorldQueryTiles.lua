...
local WorldQueryTiles = {}

WorldQueryTiles.getTileAt = function(self, x, y)
...
WorldQueryTiles.isSolidAt = function(self, x, y)
	if not self.layer or not self.layer.collision then
		return false
	end
...
WorldQueryTiles.collectTilePositions = function(self, tile_ids, rule)
	rule = rule or {}
...
WorldQueryTiles.findSurfacePositionAtX = function(self, x, radius, rule)
	rule = rule or {}
